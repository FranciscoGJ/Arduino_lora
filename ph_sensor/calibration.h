...
#define SensorPin A0
#define VREF 5000 //for arduino uno, the ADC reference is the pow er(AVCC), that is 5000mV 
...
void loop()
{
 if(serialDataAvailable() > 0)
 {
    ...
    EEPROM_read(SlopeValueAddress, slopeValue); // After cal ibration, the new slope and intercept should be read ,to update current value.
 ...
 if(millis()-sampleTimepoint>40U)
 {
    ...
    if(millis()-printTimepoint>1000U)
    {
        ...
        if(enterCalibrationFlag) // in calibration mode,print the voltage to user, to watch the stability of voltage
        {
            ...
            Serial.print("pH:"); // in normal mode, print th e ph value to user
